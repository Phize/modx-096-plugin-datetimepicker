...
function DateTimepicker() {
	...
	this._mainDivId = 'ui-datetimepicker-div'; // The ID of the main datetimepicker division
	this._appendClass = 'ui-datetimepicker-append'; // The name of the append marker class
	this._triggerClass = 'ui-datetimepicker-trigger'; // The name of the trigger marker class
	this._dialogClass = 'ui-datetimepicker-dialog'; // The name of the dialog marker class
	this._promptClass = 'ui-datetimepicker-prompt'; // The name of the dialog prompt marker class
	this._unselectableClass = 'ui-datetimepicker-unselectable'; // The name of the unselectable cell marker class
	this._currentClass = 'ui-datetimepicker-current-day'; // The name of the current day marker class
...
$.extend(DateTimepicker.prototype, {
	...
	_newInst: function(target, inline) {
		return {id: target[0].id, input: target, // associated target
			...
			$('<div class="ui-datetimepicker-inline"></div>'))};
	},
	...
	_inlineShow: function(inst) {
		...
		inst.dpDiv.width(numMonths[1] * $('.ui-datetimepicker', inst.dpDiv[0]).width());
	}, 
	...
	_doKeyDown: function(e) {
		...
		if ($.datetimepicker._datetimepickerShowing)
			switch (e.keyCode) {
				...
				case 13: $.datetimepicker._selectDay(e.target, inst.selectedMonth, inst.selectedYear,
							$('td.ui-datetimepicker-days-cell-over', inst.dpDiv)[0]);
						return false; // don't submit the form
						break; // select the value on enter
				case 27: $.datetimepicker._hideDateTimepicker(null, $.datetimepicker._get(inst, 'duration'));
						break; // hide on escape
				case 33: $.datetimepicker._adjustDate(e.target, (e.ctrlKey ? -1 :
							-$.datetimepicker._get(inst, 'stepMonths')), (e.ctrlKey ? 'Y' : 'M'));
						break; // previous month/year on page up/+ ctrl
				case 34: $.datetimepicker._adjustDate(e.target, (e.ctrlKey ? +1 :
							+$.datetimepicker._get(inst, 'stepMonths')), (e.ctrlKey ? 'Y' : 'M'));
						break; // next month/year on page down/+ ctrl
				case 35: if (e.ctrlKey) $.datetimepicker._clearDate(e.target);
						break; // clear on ctrl+end
				case 36: if (e.ctrlKey) $.datetimepicker._gotoToday(e.target);
						break; // current on ctrl+home
				case 37: if (e.ctrlKey) $.datetimepicker._adjustDate(e.target, -1, 'D');
						break; // -1 day on ctrl+left
				case 38: if (e.ctrlKey) $.datetimepicker._adjustDate(e.target, -7, 'D');
						break; // -1 week on ctrl+up
				case 39: if (e.ctrlKey) $.datetimepicker._adjustDate(e.target, +1, 'D');
						break; // +1 day on ctrl+right
				case 40: if (e.ctrlKey) $.datetimepicker._adjustDate(e.target, +7, 'D');
	...
	_doKeyPress: function(e) {
		var inst = $.data(e.target, PROP_NAME);
		var chars = $.datetimepicker._possibleChars($.datetimepicker._get(inst, 'dateFormat')+' '+$.datetimepicker._get('timeFormat'));
	...
	_showDateTimepicker: function(input) {
		...
		inst.dpDiv.width($.datetimepicker._getNumberOfMonths(inst)[1] *
			$('.ui-datetimepicker', inst.dpDiv[0])[0].offsetWidth);
		...
		if (!inst.inline) {
			...
			var postProcess = function() {
				...
				if ($.browser.msie && parseInt($.browser.version) < 7) // fix IE < 7 select problems
					$('iframe.ui-datetimepicker-cover').css({width: inst.dpDiv.width() + 4,
	...
	_updateDateTimepicker: function(inst) {
		var dims = {width: inst.dpDiv.width() + 4,
			height: inst.dpDiv.height() + 4};
		inst.dpDiv.empty().append(this._generateDateTimepicker(inst)).
			find('iframe.ui-datetimepicker-cover').
			css({width: dims.width, height: dims.height});
		var numMonths = this._getNumberOfMonths(inst);
		inst.dpDiv[(numMonths[0] != 1 || numMonths[1] != 1 ? 'add' : 'remove') +
			'Class']('ui-datetimepicker-multi');
		inst.dpDiv[(this._get(inst, 'isRTL') ? 'add' : 'remove') +
			'Class']('ui-datetimepicker-rtl');
	...
	_tidyDialog: function(inst) {
		inst.dpDiv.removeClass(this._dialogClass).unbind('.ui-datetimepicker');
	...
	/* Action for selecting a new time. */
	_selectTime: function(id, select, period) {
		...
		$('td.datetimepicker-current-day').each(function(){
			$.datetimepicker._selectDay(inst, inst.selectedMonth, inst.selectedYear, $(this));
		});
	...
	_clickTime: function(id) {
		var target = $(id);
		var inst = $.data(target[0], PROP_NAME);
		if (inst.input && inst._selectingTime && !$.browser.msie)
			inst.input[0].focus();
	...
	_selectDay: function(id, month, year, td) {
		...
		if (rangeSelect) {
			...
			if (this._stayOpen) {
				$('.ui-datetimepicker td').removeClass(this._currentClass);
	...
	_selectDate: function(id, dateStr) {
		...
		if (this._get(inst, 'rangeSelect') && dateStr)
			dateStr = (inst.rangeStart ? this._formatDateTime(inst, inst.rangeStart) :
	...
	dateStatus: function(date, inst) {
		return $.datetimepicker.formatDate($.datetimepicker._get(inst, 'dateStatus'),
			date, $.datetimepicker._getFormatConfig(inst));
	},
	...
	_setDateFromField: function(inst) {
		var dateFormat = this._get(inst, 'dateFormat')+' '+this._get(inst, 'timeFormat');
	...
	_setDate: function(inst, date, endDate) {
		...
		this._adjustInstDate(inst);
		if (inst.input)
			inst.input.val(clear ? '' : this._formatDateTime(inst) +
				(!this._get(inst, 'rangeSelect') ? '' : this._get(inst, 'rangeSeparator') +
				this._formatDateTime(inst, inst.endDay, inst.endMonth, inst.endYear)));
	},
	...
	_generateDateTimepicker: function(inst) {
		var today = new Date();
		...
		var clear = (this._get(inst, 'mandatory') ? '' :
			'<div class="ui-datetimepicker-clear"><a onclick="modxPluginDateTimepicker.jQuery.datetimepicker._clearDate(\'#' + inst.id + '\');"' +
			(showStatus ? this._addStatus(inst, this._get(inst, 'clearStatus') || '&#xa0;') : '') + '>' +
			this._get(inst, 'clearText') + '</a></div>');
		var controls = '<div class="ui-datetimepicker-control">' + (isRTL ? '' : clear) +
			'<div class="ui-datetimepicker-close"><a onclick="modxPluginDateTimepicker.jQuery.datetimepicker._hideDateTimepicker();"' +
		...
		var prev = '<div class="ui-datetimepicker-prev">' + (this._canAdjustMonth(inst, -1, drawYear, drawMonth) ? 
			'<a onclick="modxPluginDateTimepicker.jQuery.datetimepicker._adjustDate(\'#' + inst.id + '\', -' + stepMonths + ', \'M\');"' +
		...
		var next = '<div class="ui-datetimepicker-next">' + (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ?
			'<a onclick="modxPluginDateTimepicker.jQuery.datetimepicker._adjustDate(\'#' + inst.id + '\', +' + stepMonths + ', \'M\');"' +
		...
		var html = (prompt ? '<div class="' + this._promptClass + '">' + prompt + '</div>' : '') +
			...
			'<div class="ui-datetimepicker-links">' + (isRTL ? next : prev) +
			(this._isInRange(inst, (this._get(inst, 'gotoCurrent') && inst.currentDay ?
			currentDate : today)) ? '<div class="ui-datetimepicker-current">' +
			'<a onclick="modxPluginDateTimepicker.jQuery.datetimepicker._gotoToday(\'#' + inst.id + '\');"' +
		...
		for (var row = 0; row < numMonths[0]; row++)
			for (var col = 0; col < numMonths[1]; col++) {
				...
				for (var dow = 0; dow < 7; dow++) { // days of the week
					...
					html += '<td' + ((dow + firstDay + 6) % 7 >= 5 ? ' class="ui-datetimepicker-week-end-cell"' : '') + '>' +
						(!changeFirstDay ? '<span' :
						'<a onclick="modxPluginDateTimepicker.jQuery.datetimepicker._changeFirstDay(\'#' + inst.id + '\', ' + day + ');"') + 
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					html += '<tr class="ui-datetimepicker-days-row">' +
						(showWeeks ? '<td class="ui-datetimepicker-week-col">' + calculateWeek(printDate) + '</td>' : '');
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						html += '<td class="ui-datetimepicker-days-cell' +
							((dow + firstDay + 6) % 7 >= 5 ? ' ui-datetimepicker-week-end-cell' : '') + // highlight weekends
							(otherMonth ? ' ui-datetimepicker-otherMonth' : '') + // highlight days from other months
							(printDate.getTime() == selectedDate.getTime() && drawMonth == inst.selectedMonth ?
							' ui-datetimepicker-days-cell-over' : '') + // highlight selected day
							(unselectable ? ' ' + this._unselectableClass : '') +  // highlight unselectable days
							(otherMonth && !showOtherMonths ? '' : ' ' + daySettings[1] + // highlight custom dates
							(printDate.getTime() >= currentDate.getTime() && printDate.getTime() <= endDate.getTime() ?  // in current range
							' ' + this._currentClass : '') + // highlight selected day
							(printDate.getTime() == today.getTime() ? ' ui-datetimepicker-today' : '')) + '"' + // highlight today (if different)
							((!otherMonth || showOtherMonths) && daySettings[2] ? ' title="' + daySettings[2] + '"' : '') + // cell title
							(unselectable ? (highlightWeek ? ' onmouseover="modxPluginDateTimepicker.jQuery(this).parent().addClass(\'ui-datetimepicker-week-over\');"' + // highlight selection week
							' onmouseout="modxPluginDateTimepicker.jQuery(this).parent().removeClass(\'ui-datetimepicker-week-over\');"' : '') : // unhighlight selection week
							' onmouseover="modxPluginDateTimepicker.jQuery(this).addClass(\'ui-datetimepicker-days-cell-over\')' + // highlight selection
							(highlightWeek ? '.parent().addClass(\'ui-datetimepicker-week-over\')' : '') + ';' + // highlight selection week
							(!showStatus || (otherMonth && !showOtherMonths) ? '' : 'modxPluginDateTimepicker.jQuery(\'#ui-datetimepicker-status-' +
							inst.id + '\').html(\'' + (dateStatus.apply((inst.input ? inst.input[0] : null),
							[printDate, inst]) || '&#xa0;') +'\');') + '"' +
							' onmouseout="modxPluginDateTimepicker.jQuery(this).removeClass(\'ui-datetimepicker-days-cell-over\')' + // unhighlight selection
							(highlightWeek ? '.parent().removeClass(\'ui-datetimepicker-week-over\')' : '') + ';' + // unhighlight selection week
							(!showStatus || (otherMonth && !showOtherMonths) ? '' : 'modxPluginDateTimepicker.jQuery(\'#ui-datetimepicker-status-' +
							inst.id + '\').html(\'&#xa0;\');') + '" onclick="modxPluginDateTimepicker.jQuery.datetimepicker._selectDay(\'#' +
		...
		html += (showStatus ? '<div style="clear: both;"></div><div id="ui-datetimepicker-status-' + inst.id + 
			...
			'<iframe src="javascript:false;" class="ui-datetimepicker-cover"></iframe>' : '');
		return html;
	},
	...
	_generateMonthYearHeader: function(inst, drawMinute, drawHour, drawMonth, drawYear, minDate, maxDate,
		...
		if (secondary || !this._get(inst, 'changeMonth'))
			...
			html += '<select class="ui-datetimepicker-new-month" ' +
				'onchange="modxPluginDateTimepicker.jQuery.datetimepicker._selectMonthYear(\'#' + inst.id + '\', this, \'M\');" ' +
		...
		if (secondary || !this._get(inst, 'changeYear'))
			...
			html += '<select class="ui-datetimepicker-new-year" ' +
				'onchange="modxPluginDateTimepicker.jQuery.datetimepicker._selectMonthYear(\'#' + inst.id + '\', this, \'Y\');" ' +
				'onclick="modxPluginDateTimepicker.jQuery.datetimepicker._clickMonthYear(\'#' + inst.id + '\');"' +
		...
		// if (this._get('changeTime'))
		{
			...
			html += '<select class="ui-datetimepicker-new-hour" ' +
				...
				'onclick="modxPluginDateTimepicker.jQuery.datetimepicker._clickMonthYear(\'#' + inst.id + ');"' +
				(showStatus ? this._addStatus(this._get(inst, 'hourStatus') || '&#xa0;') : '') + '>';
			...
			html += '</select>';
			html += '&nbsp;:&nbsp;';
			html += '<select class="ui-datetimepicker-new-minute" ' +
				'onchange="modxPluginDateTimepicker.jQuery.datetimepicker._selectTime(\'#' + inst.id + ', this, \'M\');" ' +
				'onclick="modxPluginDateTimepicker.jQuery.datetimepicker._clickMonthYear(\'#' + inst.id + ');"' +
				(showStatus ? this._addStatus(this._get(inst, 'minuteStatus') || '&#xa0;') : '') + '>';
	...
	_addStatus: function(inst, text) {
		return ' onmouseover="modxPluginDateTimepicker.jQuery(\'#ui-datetimepicker-status-' + inst.id + '\').html(\'' + text + '\');" ' +
			'onmouseout="modxPluginDateTimepicker.jQuery(\'#ui-datetimepicker-status-' + inst.id + '\').html(\'&#xa0;\');"';
	},
...
$(document).ready(function() {
	$(document.body).append($.datetimepicker.dpDiv).
